...
int main(int argc, char** argv)
{
	...
	float confThreshold = 0.5;// parser.get<float>("thr");
	float nmsThreshold = 0.4;// parser.get<float>("nms");
	int width = 320;// parser.get<int>("width");
	int height = 320;// parser.get<int>("height");
	int imreadRGB = 0;// parser.get<int>("RGBInput");
	String detModelPath = "C:/OpenCV_2/frozen_east_text_detection/frozen_east_text_detection.pb";// parser.get<String>("detModel");
	String recModelPath = "C:/OpenCV_2/trained_model_for_text_recognition/CRNN_VGG_BiLSTM_CTC.onnx";// parser.get<String>("recModel");
	String vocPath = "C:/OpenCV_2/trained_model_for_text_recognition/alphabet_36.txt";// parser.get<String>("vocabularyPath");
	if (!parser.check())
	{
		parser.printErrors();
		return 1;
	...
	//Mat frame =imread("c:/opencv/image_text1.png", IMREAD_COLOR);// Mat frame;
	Mat frame;
	//cvtColor(img1,frame, COLOR_BGR2GRAY);*/
