...
{
    // Load the image
    Mat imgOriginal = imread("c:/opencv/redcar.jpg");

    //show the original image
    const char* pzOriginalImage = "Original Image";
    namedWindow(pzOriginalImage, WINDOW_AUTOSIZE);// CV_WINDOW_AUTOSIZE);
    imshow(pzOriginalImage, imgOriginal);

    const char* pzRotatedImage = "Rotated Image";
    namedWindow(pzRotatedImage, WINDOW_AUTOSIZE);// CV_WINDOW_AUTOSIZE);
